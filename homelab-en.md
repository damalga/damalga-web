...
A while back I had a [QNAP](https://www.qnap.com/es-es/product/ts-433), in other words a **NAS** (Network Attached Storage), a mini home server for storing files and reaching them from any device on your network (or from outside it, if you set that up). Basically, it's like having your own Google Drive/Dropbox... I used it for music, movies, that sort of thing.
...
I grabbed an old 2015 HP laptop, installed [Debian](https://www.debian.org/index.es.html) 12 and [XFCE](https://www.xfce.org/) (a pretty lightweight desktop environment) on it, and got to work: [Nextcloud](https://nextcloud.com/es/) for my own file cloud, [Navidrome](https://www.navidrome.org/) to build a private "Spoti" for streaming all my music, and [Tailscale](https://tailscale.com/) to reach everything from wherever I am, without wrestling with crazy VPNs or open ports on the router. [Here](https://hackeed.es/en/blog/post_2) I get into all the details.
...
The HP did its job, but a laptop moonlighting as a server is pretty overmotherfuckinghead. So when I discovered the [ZimaBlade](https://shop.zimaspace.com/es/products/zimablade-single-board-server-for-cyber-native), I decided to "retire" it.
...
It came stock with **Debian Bookworm** plus a visual web interface ([CasaOS](https://casaos.zimaspace.com/)) that **IceWhale** (the company behind this machine) stopped maintaining, a genuinely useful tool that makes visually managing containers a lot easier, for hosting a huge stack of programs and services running efficiently.
...
**Cockpit**, a web panel for managing the server from your browser: disks, services, logs, updates. The terminal is still boss, but for a quick check from your phone, say, it's pretty deluxe.

**Navidrome**, but this time in a container, unlike on the old HP where I installed it bare-metal… This time I skipped Nextcloud; for now I just need to get files up there, and rsync (a command for copying and syncing files between machines) handles that, end of story.

And **Tailscale**, which already showed up in the intro and deserves another bar. It's a VPN (Virtual Private Network, a private network between your devices) but without the usual headache that comes with one, meaning no opening ports or hand-editing config files. You install the app on each device, log in, and they all see each other as if they were in the same room, wherever you are. Thanks to that I can hit play on my music from the car, or check in on Zakarias from another country, all encrypted and without exposing anything to the internet.
...
[OpenWrt](https://openwrt.org/) is a full GNU/Linux system built for routers. Its own package manager (opkg), its services, a configurable firewall, SSH... GL.iNet routers ship with OpenWrt out of the box.

**Federico** is a [GL.iNet Flint 2](https://hackeed.es/product/gl-inet-flint-2-70d66428-24ad-4780-ade6-82bc6bcd5ad1), and it's the home router. It hands out addresses, routes traffic, holds up the firewall. And it runs [AdGuard Home](https://adguard.com/), a filtering DNS server: it intercepts the question "what IP does this domain resolve to?", and if the domain is on an ads-or-tracking blocklist (you can configure those lists easily from its admin panel), it answers that the domain doesn't exist, and the connection never gets made.

I already wrote on the Hackeed blog about [how to set up Pi-hole on an old Raspberry Pi](https://hackeed.es/en/blog/post_3). [Pi-hole](https://pi-hole.net/) can also be installed straight on the router, no problem. The difference is that AdGuard Home installs from Federico's own admin panel in a couple of clicks, and overall I find the experience more polished than with Pi-hole. Moving the "filter" over to Federico meant taking one machine out of the equation.

On the other hand, **Berta** is a [GL.iNet Beryl AX](https://hackeed.es/product/gl-inet-beryl-ax-fb8219f0-f26e-40cf-8bfa-fa7cbbcf8604), the travel router. It fits in the palm of your hand, plugs in anywhere, and spins up its own network wherever it is. I usually run it off my phone's connection, but if you'd rather use the hotel's, the airport's, or the library's, that's on you.

Between Federico and Berta there's a [WireGuard](https://www.wireguard.com/) tunnel; that's a VPN protocol. The difference with Tailscale comes down to layers: Tailscale is a service built on top of WireGuard that handles the hard parts for you (finding devices, managing keys, punching through NAT) in exchange for depending on its infrastructure. Plain WireGuard means you're the one generating keys and writing the config by hand. A bit more work, but zero middlemen.
...
Alba is a [Raspberry Pi 4 8GB](https://hackeed.es/product/raspberry-pi-4-model-b-8gb-695f22d8-4692-4b01-8a63-7b7cb10d32f2) in a [DeskPi Pro V3](https://hackeed.es/product/deskpi-pro-v3-0-carcasa-aluminio-para-raspberry-pi-4-e5f6a7b8-9c0d-1e2f-3a4b-5c6d7e8f9a0b) case, running Debian with no desktop environment. It started out as an observability node, with [Prometheus](https://prometheus.io/), [Grafana](https://grafana.com/), and [Loki](https://grafana.com/oss/loki/) keeping watch over the rest of the homelab, hence the name: Alba, Latin for dawn, the thing that clears up before the rest of the day does… I dunno... don't grill me about it. XD.

Over time Alba stopped just watching and started serving too. These days, on top of keeping an eye on the homelab, it also hosts a site out on the internet: [meowrawr.org](https://meowrawr.org/), a wiki about wild cats. Nginx takes the request, and a Cloudflare tunnel gets it there without needing to open a single port on Federico. I get into exactly how that's set up [here](https://hackeed.es/en/blog/post_5).

Lastly, I installed [Matomo](https://matomo.org/), a sort of open-source Google Analytics: if you've got a server (like I do), you can run it for free, and sure, if you ever want to extend its capabilities or use their hosted infrastructure, that comes with a cost, but here you're not the product. The tool itself is brilliant, and I use it partly for security (relax, no shady cookies involved) across my own sites: **Damalga**, meaning this very site, plus [Alcarcia](https://www.alcarcia.com/), [Polymorphism](https://polymorphismrecords.com/), [Meowrawr](https://meowrawr.org/), and of course [Hackeed](https://hackeed.es/), which on top of letting me spot suspicious connections, lets me see which products get searched the most. Right now the ClockworkPi uConsole is the most popular one. Which also happens to be one of the two stars of the Epilogue.

## Epilogue: Hugo and Martin

**Hugo** is a [uConsole](https://hackeed.es/product/uconsole-kit-rpi-cm4-lite-color-negro-f7a23c89-4d12-4e8a-b623-9c1d8f5a7e42) from [ClockworkPi](https://www.clockworkpi.com/). No mods for now, straight out of the box, aside from a Sway config (the window manager) I've put a fair bit of care into. You can grab the dotfiles [here](https://codeberg.org/hackeed/uconsole-dotfiles/). It's the pocket machine for tinkering, with no bigger ambition than learning the noble art of pentesting bit by bit, so I installed [Kali Linux](https://www.kali.org/) on it and kept it lean, so I don't overwhelm myself with tools I'm not going to touch for now.

Martin is more or less the same idea in a different body: a [Raspberry Pi 4 4GB](https://hackeed.es/product/raspberry-pi-4-model-b-4gb-0c40723b-461f-4c05-9aef-43233ff1c875) instead of a [Compute Module 4](https://www.raspberrypi.com/products/compute-module-4/) (which is Hugo's brain), sitting on a [52Pi battery carrier](https://hackeed.es/product/ups-con-rtc-y-coulometro-para-raspberry-pi-4b-3b-3b-a7b8c9d0-1e2f-3a4b-5c6d-7e8f9a0b1c2d), with a [HAT from the same brand stacked on top for 4G LTE connectivity](https://hackeed.es/product/hat-4g-3g-para-raspberry-pi-f6a7b8c9-0d1e-2f3a-4b5c-6d7e8f9a0b1c). The modem I fitted is an [Arduino](https://docs.arduino.cc/resources/datasheets/TPX00200-TPX00201-datasheet.pdf) in Mini PCIe format (any compatible one works here), linked to the Pi with a very short USB cable, plus its antenna, a SIM card, and on top of all that, an [Alfa Network AWUS036ACS](https://hackeed.es/product/alfa-awus036acs-dc2f61b4-5359-458a-9c79-ce2e71ffbc8d), the smallest go-to card for wifi auditing.
